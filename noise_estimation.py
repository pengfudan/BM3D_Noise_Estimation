...
def noise_estimate(im, pch_size=8):
    ...
    for ii in range(-1, -d-1, -1):
        ...
        if np.sum(sig_value[:ii] > tau) == np.sum(sig_value[:ii] < tau):  # the median of the set
            return np.sqrt(tau)
...
if __name__ == '__main__':
    ...
    for level in noise_level:
        ...
        img_noise = im_noise * 255
        img_noise = img_noise.astype(np.uint8)
        plt.figure()
        plt.imshow(img_noise)
        imwrite('./data_with_noise/lena_%d.png' % level, img_noise[:, :, [2, 1, 0]])
